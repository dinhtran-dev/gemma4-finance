...
3. Save it into this `ios/` directory (so the `.xcodeproj` lives next to the
   Swift source files already checked in).
4. In the navigator, delete Xcode's default `ContentView.swift` and
   `ExpenseParserApp.swift`, then **drag in** the Swift files from this folder:
   ...
   Choose **"Copy items if needed" = off** and **"Add to target: ExpenseParser"**.
...
## File layout (what's in this folder)
